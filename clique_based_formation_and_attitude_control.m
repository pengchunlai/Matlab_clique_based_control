...
%%----6 agents----%%
    ...
    theta = 5*rand(1,n);%[1.6 0.8 -0.9 0 -0.75 0];% 6 angle between y axis of local frame of agents and global frame
	x=zeros(2,n,iteration);%initialize the location of agents
    x(:,:,1)=7*randn(2,n);%1.5*[-4 -4 -4 4 4 4;-4 -3 -0 0 2 5];%%[-3 -2 3 -1;-1 -1.5 -1 1];
%%----6 agents----%%
...
t=0.01;%time interval for iteration
    ...
    for i=1:n
        ...
        for j=1:max(row) %cliuqe j
            ...
            for z=1:cl_n(2)%cl_n(2) is the number of agents in clique j
                ...
                matrix_y=matrix_y+b_star(:,clique_xk)*transpose(Rk_i(:,:,z)*b);
            end
            ...
            ss=transpose((b-Rc*b_star(:,i))*transpose(b))-((b-Rc*b_star(:,i))*transpose(b));
        ...
        probot(:,:,i)=[0 -1;1 0]*R(:,:,i,k)*probot0+kron(x(:,i,k),[1 1 1 1 1 1]);%motion of the shape of robots
