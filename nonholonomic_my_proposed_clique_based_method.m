...
for k=1:iteration
    for i=1:n
        ...
        s=s-(([1 0;0 1]-b*transpose(b))*phi*transpose(b)-transpose(([1 0;0 1]-b*transpose(b))*phi*transpose(b)));
        norm_phi(i,k)=norm(phi);
        %--------------------------------------
        v=R(:,:,i,k)*b*transpose(b)*phi+R(:,:,i,k)*b*transpose(b)*([1 0;0 1]-Rc*R_star(:,:,i))*b;%velocity of robot n
